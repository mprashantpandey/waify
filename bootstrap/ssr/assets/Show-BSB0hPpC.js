...
function SupportShow({
  ...
  const submit = (e) => {
    e.preventDefault();
    post(route("app.support.message", { workspace: workspace.slug, thread: thread.slug ?? thread.id }), {
      forceFormData: true,
      onSuccess: () => reset()
    });
  };
  const closeThread = () => {
    post(route("app.support.close", { workspace: workspace.slug, thread: thread.slug ?? thread.id }));
  };
            ...
            /* @__PURE__ */ jsxs("div", { className: "flex justify-end", children: [
              ...
              thread.status === "closed" && /* @__PURE__ */ jsx(
                Button,
                {
                  type: "button",
                  variant: "secondary",
                  onClick: () => post(route("app.support.reopen", { workspace: workspace.slug, thread: thread.slug ?? thread.id })),
                  className: "mr-2",
