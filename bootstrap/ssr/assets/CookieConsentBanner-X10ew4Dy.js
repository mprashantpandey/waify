...
const CONSENT_KEY$1 = "zyptos.cookie-consent";
const LEGACY_CONSENT_KEY$1 = "waify.cookie-consent";
const CONSENT_EVENT$1 = "zyptos:cookie-consent";
...
function AnalyticsScripts() {
  ...
  useEffect(() => {
    ...
    const stored = window.localStorage.getItem(CONSENT_KEY$1) ?? window.localStorage.getItem(LEGACY_CONSENT_KEY$1);
    ...
    window.addEventListener(CONSENT_EVENT$1, handler);
    window.addEventListener(LEGACY_CONSENT_EVENT, handler);
    return () => {
      window.removeEventListener(CONSENT_EVENT$1, handler);
      window.removeEventListener(LEGACY_CONSENT_EVENT, handler);
    };
  }, [consentRequired]);
  useEffect(() => {
