...
What it does:
- resets tracked Vite/SSR build artifacts to the current committed state
- removes untracked hashed assets generated by local builds
- performs a fast-forward-only pull

This is the safe path for live servers where `npm run build` is executed on the server after deployment.
