...
export default function Docs() {
    ...
    return (
        <MarketingLayout page="docs" wide>
            <Head title="Docs" />
            <div className="grid gap-6 lg:grid-cols-4">
                <Card className="h-fit p-3 lg:col-span-1">
                    {sections.map((item) => (
                        <div key={item.id} className="mb-4 last:mb-0">
                            <p className="mb-1 px-2 text-[10px] font-bold uppercase tracking-wider text-waify-text-muted">{item.title}</p>
                            {item.pages.map((doc) => (
                                <button
                                    key={doc.id}
                                    type="button"
                                    onClick={() => { setSectionId(item.id); setPageId(doc.id); }}
                ...
                <Card className="p-6 sm:p-8 lg:col-span-3">
                    <p className="mb-1 text-xs text-waify-text-muted">{section.title}</p>
                    <h2 className="text-2xl font-bold">{page.title}</h2>
                    <p className="mt-4 whitespace-pre-wrap text-sm leading-relaxed text-waify-text-muted dark:text-waify-dark-text-muted">{page.body}</p>
                    <div className="mt-8 rounded-xl bg-gray-50 p-4 text-xs leading-relaxed text-waify-text-muted dark:bg-slate-800/50">
                        These docs are a public quick reference. For workspace-specific setup, use the in-app diagnostics, integration logs, billing preview, and automation test runner.
                    </div>
                    <div className="mt-6 flex flex-wrap gap-2">
                        <Link href={route('knowledgebase')}><Button variant="secondary" size="sm">Knowledge base</Button></Link>
                        <Link href={route('app.dashboard')}><Button variant="secondary" size="sm">Open app</Button></Link>
                    </div>
                </Card>
            </div>
        </MarketingLayout>
