...
function PlatformSupportShow({
  ...
  const normalizedInitialMessages = useMemo(
    () => (Array.isArray(messages) ? messages : []).map(normalizeMessage).filter((message) => message !== null),
    [messages]
  );
  const [items, setItems] = useState(normalizedInitialMessages);
  ...
  const hydratedThreadIdRef = useRef(null);
  useEffect(() => {
    const incoming = (Array.isArray(messages) ? messages : []).map(normalizeMessage).filter((message) => message !== null);
  ...
  const mergeIncomingMessages = useCallback((incomingRaw) => {
    const incoming = Array.isArray(incomingRaw) ? incomingRaw.map(normalizeMessage).filter((message) => message !== null) : [];
    if (incoming.length === 0) {
      return;
    }
    setItems((prev) => mergeMessages(prev, incoming));
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribe(channel, "support.message.created", (payload) => {
      if (payload?.thread_id && Number(payload.thread_id) !== thread.id) {
        return;
      }
    ...
    const poll = async () => {
      try {
        const res = await axios.get(
          route("platform.support.live.thread", { thread: thread.slug ?? thread.id }),
          { headers: { Accept: "application/json" } }
        );
        const data2 = res.data;
        if (!data2?.thread?.id || Number(data2.thread.id) !== thread.id) {
          return;
        }
        mergeIncomingMessages(data2.messages ?? []);
      } catch (_error) {
      }
    };
  ...
  return /* @__PURE__ */ jsxs(PlatformShell, { children: [
    ...
    /* @__PURE__ */ jsxs("div", { className: "space-y-6", children: [
      /* @__PURE__ */ jsxs("div", { children: [
        ...
        thread.account && /* @__PURE__ */ jsxs("div", { className: "mt-2 flex flex-wrap items-center gap-2 text-xs text-gray-600 dark:text-gray-300", children: [
          /* @__PURE__ */ jsxs("span", { children: [
            "Owner: ",
            thread.account.owner?.name || "—"
          ] }),
          /* @__PURE__ */ jsx("span", { children: "·" }),
          /* @__PURE__ */ jsx("span", { children: thread.account.owner?.email || "—" }),
          /* @__PURE__ */ jsx("span", { children: "·" }),
          /* @__PURE__ */ jsx(
            "a",
            {
