...
import { X, Activity, Users, Settings, LifeBuoy, CreditCard, BarChart3, Zap, Sparkles, Bot, Inbox, FileText, MessageCircle, Puzzle, LayoutDashboard, MessageSquare } from "lucide-react";
...
function Sidebar({ navigation, currentRoute, workspace, isOpen = false, onClose }) {
...
function LiveChatWidget() {
  const { workspace, branding } = usePage().props;
  ...
  useEffect(() => {
    ...
    let active = true;
    setLoading(true);
    axios.get(route("app.support.live", { workspace: workspace.slug }), {
  ...
  const sendMessage = async () => {
    ...
    const res = await axios.post(
      route("app.support.live.message", { workspace: workspace.slug }),
      payload,
      { headers: { Accept: "application/json" } }
    );
    const data = res.data;
  ...
  const closeLiveChat = async () => {
    ...
    await axios.post(
      route("app.support.live.close", { workspace: workspace.slug }),
      {},
      { headers: { Accept: "application/json" } }
    );
  ...
  const requestHuman = async () => {
    ...
    try {
      const res = await axios.post(
        route("app.support.live.request-human", { workspace: workspace.slug }),
        {},
        { headers: { Accept: "application/json" } }
      );
      const data = res.data;
...
function AppShell({ children }) {
  const { workspace, workspaces, navigation, auth, ziggy } = usePage().props;
