...
function nodeHasInlineIcon(node) {
  ...
  const hasIconClass = /\blucide\b/.test(className) || /\bh-\d+(\.\d+)?\b/.test(className) && /\bw-\d+(\.\d+)?\b/.test(className) || node.props?.["data-lucide"] !== void 0;
  if (hasIconClass) {
    return true;
  }
  return nodeHasInlineIcon(node.props?.children);
}
function Alert({ variant = "info", title, children, onClose, className }) {
  ...
  return /* @__PURE__ */ jsxs(
    "div",
    {
      ...
      role: "alert",
      children: [
        !childHasInlineIcon && /* @__PURE__ */ jsx(Icon, { className: "h-5 w-5 flex-shrink-0 mt-0.5" }),
