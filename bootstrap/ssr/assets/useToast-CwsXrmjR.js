...
function hasRecentNonFlashToast(windowMs = 2e3) {
  return Date.now() - lastNonFlashToastAt <= windowMs;
}
function normalizeToastMessage(toast) {
  const normalize = (value) => value.trim().toLowerCase().replace(/\b(successfully|successful|success)\b/g, "").replace(/\b(done|completed)\b/g, "").replace(/\bfailed to\b/g, "").replace(/\bunable to\b/g, "").replace(/\byour\b/g, "").replace(/[.!?]+$/g, "").replace(/\s+/g, " ").trim();
...
function useToast() {
  ...
  const addToast = useCallback((toast2) => {
    ...
    for (const [seenSignature, seenAt] of recentEventSignatures.entries()) {
      ...
      if (seenSignature && eventSignature && (seenSignature.includes(eventSignature) || eventSignature.includes(seenSignature))) {
